...
export function generateURLObjectsBlog(paths: OstDocument<{ [key: string]: unknown; }>[], host: string): URLObject[] {
  return paths.map((url: any) => {
    return ({
...
export function generateURLObjectsWnioski(paths: OstDocument<{ [key: string]: unknown; }>[], host: string): URLObject[] {
  return paths.map((url: any) => {
