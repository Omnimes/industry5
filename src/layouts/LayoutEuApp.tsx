      ...
      {posts?.length ? (
        <div className="flex w-full flex-col gap-8 pt-2 sm:grid md:grid-cols-2 lg:grid-cols-3">
          {sortedPosts.map((post, index) => {
            ...
            return (
              <MyLinkChildren href={`/${basePath}/${post.slug}`} aria-label={taria("readMoreAria")} text={t("ReadMore")} key={post.title} className={cn(
                ...
                <div className="relative mt-6 p-4">
                  <h3 className="mb-2 text-lg font-bold text-gray-800 md:min-h-[64px] md:text-xl dark:text-white">{post.title}</h3>
                  <p className="line-clamp-3 text-sm text-gray-600 md:text-base dark:text-gray-300">{post.description}</p>
                </div>
                <span className="mt-auto w-full px-4 pb-4 dark:text-gray-300 hover:dark:text-white">{t("ReadMore")} &rarr;</span>
              </MyLinkChildren>
            )})}
        </div>
      ) : (
